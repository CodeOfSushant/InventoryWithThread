...
public class InventoryThread {
    ...
    private static final BlockingQueue<Product> rawItems = new LinkedBlockingQueue<>();
    // Final collection for processed items
    private static final List<Product> processedItems = new ArrayList<>();
    // Poison pill to signal end of data
    private static final Product POISON_PILL = new Product("EOF", "EOF", 0);
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread dbReader = new Thread(() -> {
            ...
            try (Connection conn = DriverManager.getConnection(url, user, password);
                ...
                while (rs.next()) {
                    Product item = new Product(rs.getString("Product_Name"), rs.getString("Product_Type"), rs.getDouble("Product_Price"));
        ...
        Thread taxProcessor = new Thread(() -> {
            try {
                while (true) {
                    Product item = rawItems.take(); // Fetch already created object
                    if (item == POISON_PILL) {
                        break;
                    }
